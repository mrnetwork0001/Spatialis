...
- Lens Studio uses a right-handed coordinate system: +X cross +Y gives +Z. The vec3 constant `forward` is defined as `(0, 0, 1)` (+Z). A camera with identity rotation faces towards world −Z.
