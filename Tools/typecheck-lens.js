...
const sik = path.join(work, "Packages", "SpectaclesInteractionKit.lspkg");
fs.mkdirSync(sik, { recursive: true });
fs.writeFileSync(
  path.join(sik, "SIK.ts"),
  '// Shim for the real-API type check only: SIK ships as a project package, not in StudioLib.\n' +
...
console.log(`typecheck:lens — Lens Studio ${version}, StudioLib.d.ts (${lines.toLocaleString()} lines), ${scripts.length} scripts`);
